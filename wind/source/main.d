...
void dumpGUIDAttr(std.stdio.File f, CustomAttributeSig sig)
{
    ...
    // auto elementValue = element.value;
    // if (elementValue.type() != typeid(immutable(char)[])) {
    //     f.writefln("// ERROR @GUID(): unexpected type %s", elementValue.type().toString());
    //     return;
    // }
...
void dumpInterfaces(std.stdio.File f, const ref Metadata db, string namespace, bool docs = false)
{
    ...
    foreach(intf; getInterfaces(db, namespace))
    {
        ...
        f.writefln("// === List GuidAttribute");
        foreach(guidAttr; intf.attributes().find!(a => a.name == "GuidAttribute"))
        {
            f.writefln("//        %s: type %s value %s", guidAttr.name, guidAttr.value.fixed[0].value.get!ElementSig().value.type().toString(), guidAttr.value);
        }
        f.writefln("// === End List GuidAttribute");

        foreach(ca; intf.attributes())
        {
            f.writefln("// Attribute %s: type %s, value %s", ca.name, ca.value.fixed[0].value.get!ElementSig().value.type().toString(), ca.value);
            auto attrName = ca.name;
            if (attrName == "GuidAttribute")
            {
                f.writefln("// Found GuidAttribute ATTR: %s : %s", ca.name, ca.value);
                auto attrValueType = ca.value.fixed[0].value.get!ElementSig().value.type().toString();
                if (attrValueType == "immutable(char)[]") 
                {
